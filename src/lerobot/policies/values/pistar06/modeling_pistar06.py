...
from lerobot.policies.values.pistar06.configuration_pistar06 import Pistar06Config
from lerobot.policies.values.pistar06.processor_pistar06 import PISTAR06_IMAGE_MASK_KEY, PISTAR06_IMAGES_KEY
...
def _load_language_model(
    ...
    model_config = AutoConfig.from_pretrained(repo_id, revision=revision, trust_remote_code=True)
...
class Pistar06Policy(PreTrainedPolicy):
    ...
    def build_training_raw_batch_hook(self, dataset, targets_cfg):
        """
        Builds a hook function that computes value targets for each frame in the dataset
        based on episode-level success and remaining steps, and adds them to the training batch under the specified target key.
        """
